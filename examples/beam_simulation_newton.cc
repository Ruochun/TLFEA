/**
 * Beam Simulation with Newton Solver
 *
 * Author: GitHub Copilot Agent
 *
 * This example demonstrates loading a mesh from a VTU file and performing
 * a realistic engineering analysis using the SyncedNewton solver:
 ...
 * 3. Run a dynamic simulation using TLFEA's SyncedNewton solver
...
#include "solvers/SyncedNewton.cuh"
#include "types.h"
...
int main() {
    ...
    // inner_atol, inner_rtol, outer_tol, rho, max_outer, max_inner, time_step
    SyncedNewtonParams params = {1.0e-6, 1.0e-6, 1.0e-6, 1e14, 5, 30, 1.0e-4};

    SyncedNewtonSolver solver(&gpu_t10_data, gpu_t10_data.get_n_constraint());
    solver.Setup();
    solver.SetParameters(&params);

    std::cout << "Solver initialized: SyncedNewton with h=" << std::scientific << params.time_step
