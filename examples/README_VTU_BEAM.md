...
## Key Differences from Cantilever Beam Simulation

This example differs from `cantilever_beam_simulation.cc` in the following ways:

1. **Mesh Loading**: Uses MoPhiEssentials' `mophi::LoadVtu()` function to load a VTU file instead of reading TetGen .node/.ele files
2. **Mesh Format**: Works with VTU mesh files which can be generated by many mesh generation tools (Gmsh, Salome, etc.)
3. **Loading Type**: Applies a concentrated load at one end instead of gravity acting on the entire beam
...
The simulation generates VTK files with the naming pattern `output_vtu_beam_#####.vtk` where ##### is the timestep number (00000, 00010, 00020, etc.).
...
1. Open ParaView
2. File → Open → Select all `output_vtu_beam_*.vtk` files
...
```bash
paraview output_vtu_beam_*.vtk
```

## Simulation Parameters

You can modify the simulation by editing `examples/vtu_beam_simulation.cc`:
