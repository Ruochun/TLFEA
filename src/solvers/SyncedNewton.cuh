...
struct SyncedNewtonParams {
  Real inner_atol, inner_rtol, outer_tol, rho;
...
class SyncedNewtonSolver : public SolverBase {
 public:
  SyncedNewtonSolver(ElementBase *data, int n_constraints)
    ...
    if (d_data_ == nullptr) {
      MOPHI_ERROR("d_data_ is null in SyncedNewtonSolver constructor");
                << std::endl;
    }
    ...
    cudaMalloc(&d_delta_v_, n_coef_ * 3 * sizeof(Real));
    cudaMalloc(&d_r_, n_coef_ * 3 * sizeof(Real));
    cudaMalloc(&d_r_dot_r_, sizeof(Real));
    cudaMalloc(&d_alpha_cg_, sizeof(Real));
    cudaMalloc(&d_beta_cg_, sizeof(Real));
    ...
    cudaMalloc(&d_norm_temp_, sizeof(Real));
  ...
  void SetParameters(void *params) override {
    ...
    cudaMemcpy(d_inner_atol_, &p->inner_atol, sizeof(Real),
  ...
  void Setup() {
    ...
    cudaMemset(d_dv_, 0, n_coef_ * 3 * sizeof(Real));
    cudaMemset(d_r_dot_r_, 0, sizeof(Real));
  ...
  __device__ Map<VectorXR> dv() {
    return Map<VectorXR>(d_dv_, 3 * n_coef_);
  }
  ...
  __device__ Real solver_inner_atol() const {
  ...
  __device__ Map<VectorXR> delta_v() {
    return Map<VectorXR>(d_delta_v_, 3 * n_coef_);
  }
  __device__ Map<VectorXR> r() {
    return Map<VectorXR>(d_r_, 3 * n_coef_);
  }
  __device__ Real *r_dot_r() {
    return d_r_dot_r_;
  }
  __device__ Real *alpha_cg() {
    return d_alpha_cg_;
  }
  __device__ Real *beta_cg() {
    return d_beta_cg_;
  }
#endif
  ...
  // ..])
  Real *GetVelocityGuessDevicePtr() const {
    return d_v_guess_;
  }

  void SetInitialVelocity(const VectorXR& h_v0) {
    if (h_v0.size() != n_coef_ * 3) {
      MOPHI_ERROR("SetInitialVelocity: size mismatch (got %d, expected %d)", 
                  (int)h_v0.size(), (int)(n_coef_ * 3));
      return;
    }
    MOPHI_GPU_CALL(cudaMemcpy(d_v_guess_, h_v0.data(),
                            static_cast<size_t>(n_coef_) * 3 * sizeof(Real),
                            cudaMemcpyHostToDevice));
    MOPHI_GPU_CALL(cudaMemcpy(d_v_prev_, h_v0.data(),
                            static_cast<size_t>(n_coef_) * 3 * sizeof(Real),
                            cudaMemcpyHostToDevice));
  }

  void SetInitialVelocityFromDevicePtr(const Real* d_v0) {
    if (d_v0 == nullptr) {
      MOPHI_ERROR("SetInitialVelocityFromDevicePtr: null input pointer");
      return;
    }
    MOPHI_GPU_CALL(cudaMemcpy(d_v_guess_, d_v0,
                            static_cast<size_t>(n_coef_) * 3 * sizeof(Real),
                            cudaMemcpyDeviceToDevice));
    MOPHI_GPU_CALL(cudaMemcpy(d_v_prev_, d_v0,
                            static_cast<size_t>(n_coef_) * 3 * sizeof(Real),
                            cudaMemcpyDeviceToDevice));
  }
  }
 ...
 private:
  ...
  Real *d_v_guess_, *d_v_prev_;
  Real *d_lambda_guess_, *d_g_, *d_dv_;
  Real *d_norm_g_;
  int *d_inner_flag_, *d_outer_flag_;
  Real *d_inner_atol_, *d_inner_rtol_, *d_outer_tol_, *d_time_step_,
      *d_solver_rho_;
  Real h_inner_atol_, h_outer_tol_, h_inner_rtol_;
  int h_max_outer_, h_max_inner_;
  int *d_max_inner_, *d_max_outer_;
  Real *d_delta_v_, *d_r_;
  Real *d_r_dot_r_;                // Scalar for dot product storage
  Real *d_alpha_cg_, *d_beta_cg_;  // CG scalars
  ...
  Real *d_norm_temp_;  // Reusable temp for cuBLAS norms
