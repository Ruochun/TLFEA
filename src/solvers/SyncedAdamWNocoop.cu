...
Real SyncedAdamWNocoopSolver::compute_l2_norm_cublas(Real *d_vec,
                                                       int n_dofs) {
  cublasDnrm2(cublas_handle_, n_dofs, d_vec, 1, d_norm_temp_cublas_);

  Real h_norm = 0.0;
  MOPHI_GPU_CALL(cudaMemcpy(&h_norm, d_norm_temp_cublas_, sizeof(Real),
                          cudaMemcpyDeviceToHost));
  return h_norm;
}

template <typename ElementType>
__device__ Real solver_grad_L_nocoop(int tid, ElementType *data,
...
__global__ void adamw_reset_inner_flag_kernel(
    SyncedAdamWNocoopSolver *d_solver) {
  if (threadIdx.x == 0 && blockIdx.x == 0) {
    *d_solver->inner_flag() = 0;
    *d_solver->norm_g()     = 0.0;
...
__global__ void adamw_update_velocity_kernel(SyncedAdamWNocoopSolver *d_solver,
  ...
  Real y = v_guess_tid - lr_current * (m_hat / (sqrt(v_hat) + eps) +
                                         weight_decay * v_guess_tid);
...
__global__ void adamw_update_positions_from_prev_kernel(
  ...
  if (tid < d_solver->get_n_coef()) {
    ...
    d_data->x12()(tid) =
        d_solver->x12_prev()(tid) + dt * d_solver->v_guess()(tid * 3 + 0);
    d_data->y12()(tid) =
        d_solver->y12_prev()(tid) + dt * d_solver->v_guess()(tid * 3 + 1);
    d_data->z12()(tid) =
        d_solver->z12_prev()(tid) + dt * d_solver->v_guess()(tid * 3 + 2);
  }
}
...
template <typename ElementType>
__global__ void adamw_compute_gradient_kernel(
    ElementType *d_data, SyncedAdamWNocoopSolver *d_solver) {
  int tid = blockIdx.x * blockDim.x + threadIdx.x;
  int n   = d_solver->get_n_coef() * 3;
  if (tid < n) {
...
void SyncedAdamWNocoopSolver::OneStepAdamWNocoop() {
  ...
  MOPHI_GPU_CALL(cudaMemcpy(&lr_decay, d_lr_decay_, sizeof(Real),
                          cudaMemcpyDeviceToHost));
  MOPHI_GPU_CALL(
      cudaMemcpy(&beta1_h, d_beta1_, sizeof(Real), cudaMemcpyDeviceToHost));
  MOPHI_GPU_CALL(
      cudaMemcpy(&beta2_h, d_beta2_, sizeof(Real), cudaMemcpyDeviceToHost));
  MOPHI_GPU_CALL(cudaMemcpy(&inner_tol, d_inner_tol_, sizeof(Real),
                          cudaMemcpyDeviceToHost));
  MOPHI_GPU_CALL(cudaMemcpy(&inner_rtol, d_inner_rtol_, sizeof(Real),
                          cudaMemcpyDeviceToHost));
  MOPHI_GPU_CALL(cudaMemcpy(&outer_tol, d_outer_tol_, sizeof(Real),
                          cudaMemcpyDeviceToHost));
  MOPHI_GPU_CALL(cudaMemcpy(&max_outer, d_max_outer_, sizeof(int),
                          cudaMemcpyDeviceToHost));
  MOPHI_GPU_CALL(cudaMemcpy(&max_inner, d_max_inner_, sizeof(int),
                          cudaMemcpyDeviceToHost));
  MOPHI_GPU_CALL(cudaMemcpy(&conv_check_interval, d_convergence_check_interval_,
  ...
  auto one_step_typed = [&](auto *typed_data) {
    adamw_save_prev_pos_kernel<<<blocks_coef, threadsPerBlock>>>(
    ...
    for (int outer_iter = 0; outer_iter < effective_max_outer; outer_iter++) {
      ...
      for (int inner_iter = 0; inner_iter < max_inner; inner_iter++) {
        ...
        adamw_update_positions_from_prev_kernel<<<blocks_coef,
                                                  threadsPerBlock>>>(
            typed_data, d_adamw_solver_);
        ...
        if (n_constraints_ > 0) {
          adamw_compute_constraints_kernel<<<blocks_fixed_nodes,
        ...
        if (inner_iter % conv_check_interval == 0) {
          ...
          const Real tol_rel = (inner_rtol > 0.0 && norm_g0 > 0.0)
                                     ? (inner_rtol * norm_g0)
                                     : 0.0;
          ...
          if (abs_ok || rel_ok) {
            ...
            inner_flag_h = 1;
            MOPHI_GPU_CALL(cudaMemcpy(d_inner_flag_, &inner_flag_h, sizeof(int),
      ...
      adamw_update_v_prev_kernel<<<blocks_dof, threadsPerBlock>>>(
          d_adamw_solver_);
      ...
      if (n_constraints_ > 0) {
        ...
        if (d_constraint_ptr_ == nullptr) {
          MOPHI_ERROR("SyncedAdamWNocoop: constraint pointer is null but "
                       "n_constraints_ > 0. Constraint setup is required for "
                       "outer convergence check.");
        }

        Real norm_constraint =
        ...
        if (norm_constraint < outer_tol) {
          if (inner_flag_h != 0) {
            ...
            outer_flag_h = 1;
            MOPHI_GPU_CALL(cudaMemcpy(d_outer_flag_, &outer_flag_h, sizeof(int),
                                    cudaMemcpyHostToDevice));
          } else {
