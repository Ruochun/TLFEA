/*
 * Adapted for TLFEA project
 * Changes: Updated includes, changed double to Real, updated Eigen types to tlfea types
 */
/*==============================================================
 *==============================================================
 * Project: RoboDyna
...
__global__ void assemble_sparse_hessian_mass(ElementType *d_data,
  ...
  const Real inv_h = 1.0 / d_solver->solver_time_step();
  int node_i         = tid;

  const int *mass_offsets   = d_data->csr_offsets();
  const int *mass_columns   = d_data->csr_columns();
  const Real *mass_values = d_data->csr_values();
  ...
  for (int idx = row_start; idx < row_end; idx++) {
    ...
    Real mass_ij = mass_values[idx];
    Real contrib = mass_ij * inv_h;
...
__global__ void assemble_sparse_hessian_tangent(ElementType *d_data,
  ...
  const Real h = d_solver->solver_time_step();
...
__global__ void assemble_sparse_hessian_constraints(
    ...
    int *d_csr_col_indices, Real *d_csr_values) {
  ...
  const Real *j_values = d_data->j_csr_values();
...
__global__ void cudss_solve_update_dual_var(
  ...
  if (tid < n_constraints) {
    Real constraint_val = d_data->constraint()[tid];
...
__global__ void cudss_solve_compute_grad_l(
  ...
  if (tid < d_newton_solver->get_n_coef() * 3) {
    Real g                  = solver_grad_L(tid, d_data, d_newton_solver);
...
Real SyncedNewtonSolver::compute_l2_norm_cublas(Real *d_vec, int n_dofs) {
  // Reuse persistent cublas_handle_ and d_norm_temp_.
  cublasDnrm2(cublas_handle_, n_dofs, d_vec, 1, d_norm_temp_);

  Real h_norm;
  MOPHI_GPU_CALL(cudaMemcpy(&h_norm, d_norm_temp_, sizeof(Real),
...
void SyncedNewtonSolver::AnalyzeHessianSparsity() {
  ...
  if (type_ == TYPE_3243 && n_constraints_ > 0) {
    ...
    if (typed_data->GetConstraintMode() ==
      ...
      h_nnz_ = running;
      MOPHI_GPU_CALL(cudaMalloc(&d_csr_row_offsets_,
                              static_cast<size_t>(n_dofs + 1) * sizeof(int)));
      MOPHI_GPU_CALL(cudaMalloc(&d_csr_col_indices_,
                              static_cast<size_t>(h_nnz_) * sizeof(int)));
      MOPHI_GPU_CALL(cudaMalloc(&d_csr_values_,
                              static_cast<size_t>(h_nnz_) * sizeof(Real)));

      MOPHI_GPU_CALL(cudaMemcpy(d_csr_row_offsets_, dof_offsets.data(),
                              static_cast<size_t>(n_dofs + 1) * sizeof(int),
                              cudaMemcpyHostToDevice));
      MOPHI_GPU_CALL(cudaMemcpy(d_csr_col_indices_, dof_columns.data(),
                              static_cast<size_t>(h_nnz_) * sizeof(int),
                              cudaMemcpyHostToDevice));
      MOPHI_GPU_CALL(cudaMemset(d_csr_values_, 0,
                              static_cast<size_t>(h_nnz_) * sizeof(Real)));

      std::cout << "Sparse Hessian (constraint-aware): " << n_dofs << " x "
                << n_dofs << ", nnz = " << h_nnz_ << " ("
                << (100.0 * static_cast<Real>(h_nnz_)) /
                       (static_cast<Real>(n_dofs) *
                        static_cast<Real>(n_dofs))
                << "%)" << std::endl;
  ...
  int zero = 0;
  MOPHI_GPU_CALL(cudaMemcpy(d_csr_row_offsets_, &zero, sizeof(int),
                          cudaMemcpyHostToDevice));
  ...
  MOPHI_GPU_CALL(cudaMemcpy(d_csr_row_offsets_ + 1, d_temp_scan,
                          static_cast<size_t>(n_dofs) * sizeof(int),
                          cudaMemcpyDeviceToDevice));
  MOPHI_GPU_CALL(cudaFree(d_temp_scan));

  MOPHI_GPU_CALL(cudaMemcpy(&h_nnz_, &d_csr_row_offsets_[n_dofs], sizeof(int),
                          cudaMemcpyDeviceToHost));
  ...
  MOPHI_GPU_CALL(cudaMalloc(&d_csr_col_indices_,
                          static_cast<size_t>(h_nnz_) * sizeof(int)));
  MOPHI_GPU_CALL(
      cudaMalloc(&d_csr_values_, static_cast<size_t>(h_nnz_) * sizeof(Real)));
  ...
  MOPHI_GPU_CALL(cudaDeviceSynchronize());
  MOPHI_GPU_CALL(cudaFree(d_row_nnz));

  std::cout << "Sparse Hessian: " << n_dofs << " x " << n_dofs
            << ", nnz = " << h_nnz_ << " ("
            << (100.0 * static_cast<Real>(h_nnz_)) /
                   (static_cast<Real>(n_dofs) * static_cast<Real>(n_dofs))
            << "%)" << std::endl;
...
void SyncedNewtonSolver::OneStepNewtonCuDSS() {
  ...
  if (!analysis_done_) {
    ...
    const Real secs =
        std::chrono::duration_cast<std::chrono::duration<Real>>(t1 - t0)
            .count();
  ...
  if (type_ == TYPE_T10) {
    ...
    for (int outer_iter = 0; outer_iter < h_max_outer_; ++outer_iter) {
      ...
      for (int newton_iter = 0; newton_iter < h_max_inner_; ++newton_iter) {
        ...
        MOPHI_GPU_CALL(cudaDeviceSynchronize());
        Real norm_g = compute_l2_norm_cublas(d_g_, n_dofs);
        ...
        MOPHI_GPU_CALL(cudaMemset(d_csr_values_, 0, h_nnz_ * sizeof(Real)));
        ...
        MOPHI_GPU_CALL(cudaMemset(d_delta_v_, 0, n_dofs * sizeof(Real)));
