...
__device__ __forceinline__ void ancf3443_solve_3x3_system(double A[3][3], double b[3], double x[3]) {
...
ancf3443_b_vec_xi(double xi, double eta, double zeta, double L, double W, double H, double* out) {
    double u = L * xi / 2.0;
    double v = W * eta / 2.0;
    double w = H * zeta / 2.0;
...
__device__ __forceinline__ void
ancf3443_db_dxi(double xi, double eta, double zeta, double L, double W, double H, double* out) {
...
__device__ __forceinline__ void
ancf3443_db_deta(double xi, double eta, double zeta, double L, double W, double H, double* out) {
    double u = 0.5 * L * xi;
    double v = 0.5 * W * eta;
    double w = 0.5 * H * zeta;
...
__device__ __forceinline__ void
ancf3443_db_dzeta(double xi, double eta, double zeta, double L, double W, double H, double* out) {
    double u = 0.5 * L * xi;
    double v = 0.5 * W * eta;
...
__device__ __forceinline__ void vbd_accumulate_residual_and_hessian_diag(int elem_idx,
    ...
    const double scale = d_data->weight_xi()(qp_idx / (Quadrature::N_QP_4 * Quadrature::N_QP_3)) *
                         d_data->weight_eta()((qp_idx / Quadrature::N_QP_3) % Quadrature::N_QP_4) *
                         d_data->weight_zeta()(qp_idx % Quadrature::N_QP_3);
    const double dV = d_data->detJ_ref(elem_idx, qp_idx) * scale;
