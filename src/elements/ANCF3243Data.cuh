...
struct GPU_ANCF3243_Data : public ElementBase {
    ...
    void Initialize() {
        ...
        HANDLE_ERROR(cudaMalloc(&d_grad_N_ref,
                                n_beam * Quadrature::N_TOTAL_QP_3_2_2 * Quadrature::N_SHAPE_3243 * 3 * sizeof(double)));
        HANDLE_ERROR(cudaMalloc(&d_detJ_ref, n_beam * Quadrature::N_TOTAL_QP_3_2_2 * sizeof(double)));
        ...
        HANDLE_ERROR(cudaMalloc(&d_Fdot, n_beam * Quadrature::N_TOTAL_QP_3_2_2 * 3 * 3 * sizeof(double)));
        HANDLE_ERROR(cudaMalloc(&d_P_vis, n_beam * Quadrature::N_TOTAL_QP_3_2_2 * 3 * 3 * sizeof(double)));
    ...
    void Setup(const Eigen::VectorXd& length,
        ...
        if (length.size() != n_beam || width.size() != n_beam || height.size() != n_beam) {
            std::cerr << "GPU_ANCF3243_Data::Setup: length/width/height must have "
                         "size n_beam."
                      << std::endl;
            return;
        }
        ...
        try {
            ...
            std::cerr << "GPU_ANCF3243_Data::Setup: failed to build per-element B_inv: " << e.what() << std::endl;
            return;
        }
        ...
        HANDLE_ERROR(cudaMemcpy(d_element_connectivity, h_element_connectivity.data(), n_elements * 2 * sizeof(int),
    ...
    void SetDensity(double rho0) {
        if (!is_setup) {
            std::cerr << "GPU_ANCF3243_Data must be set up before setting density." << std::endl;
            return;
        }
        HANDLE_ERROR(cudaMemcpy(d_rho0, &rho0, sizeof(double), cudaMemcpyHostToDevice));
    }
    ...
    void SetDamping(double eta_damp, double lambda_damp) {
        if (!is_setup) {
            std::cerr << "GPU_ANCF3243_Data must be set up before setting damping." << std::endl;
            return;
        }
        HANDLE_ERROR(cudaMemcpy(d_eta_damp, &eta_damp, sizeof(double), cudaMemcpyHostToDevice));
        HANDLE_ERROR(cudaMemcpy(d_lambda_damp, &lambda_damp, sizeof(double), cudaMemcpyHostToDevice));
    ...
    void SetSVK() {
        if (!is_setup) {
            std::cerr << "GPU_ANCF3243_Data must be set up before setting material." << std::endl;
            return;
        }
    ...
    void SetNodalFixed(const Eigen::VectorXi& fixed_nodes) {
        if (is_constraints_setup) {
            std::cerr << "GPU_ANCF3243_Data CONSTRAINT is already set up." << std::endl;
            return;
        }
