...
struct GPU_ANCF3243_Data : public ElementBase {
    ...
    __device__ const Eigen::Map<VectorXR> gauss_xi_m() const {
        return Eigen::Map<VectorXR>(d_gauss_xi_m, Quadrature::N_QP_6);
    }

    __device__ const Eigen::Map<VectorXR> gauss_xi() const {
        return Eigen::Map<VectorXR>(d_gauss_xi, Quadrature::N_QP_3);
    }

    __device__ const Eigen::Map<VectorXR> gauss_eta() const {
        return Eigen::Map<VectorXR>(d_gauss_eta, Quadrature::N_QP_2);
    }

    __device__ const Eigen::Map<VectorXR> gauss_zeta() const {
        return Eigen::Map<VectorXR>(d_gauss_zeta, Quadrature::N_QP_2);
    }

    __device__ const Eigen::Map<VectorXR> weight_xi_m() const {
        return Eigen::Map<VectorXR>(d_weight_xi_m, Quadrature::N_QP_6);
    }

    __device__ const Eigen::Map<VectorXR> weight_xi() const {
        return Eigen::Map<VectorXR>(d_weight_xi, Quadrature::N_QP_3);
    }

    __device__ const Eigen::Map<VectorXR> weight_eta() const {
        return Eigen::Map<VectorXR>(d_weight_eta, Quadrature::N_QP_2);
    }

    __device__ const Eigen::Map<VectorXR> weight_zeta() const {
        return Eigen::Map<VectorXR>(d_weight_zeta, Quadrature::N_QP_2);
    }
    ...
    __device__ Eigen::Map<VectorXR> x12(int elem) {
        return Eigen::Map<VectorXR>(d_x12 + elem * (Quadrature::N_SHAPE_3243 / 2), Quadrature::N_SHAPE_3243);
    }

    __device__ Eigen::Map<VectorXR> const x12(int elem) const {
        return Eigen::Map<VectorXR>(d_x12 + elem * (Quadrature::N_SHAPE_3243 / 2), Quadrature::N_SHAPE_3243);
    }

    __device__ Eigen::Map<VectorXR> y12(int elem) {
        return Eigen::Map<VectorXR>(d_y12 + elem * (Quadrature::N_SHAPE_3243 / 2), Quadrature::N_SHAPE_3243);
    }

    __device__ Eigen::Map<VectorXR> const y12(int elem) const {
        return Eigen::Map<VectorXR>(d_y12 + elem * (Quadrature::N_SHAPE_3243 / 2), Quadrature::N_SHAPE_3243);
    }

    __device__ Eigen::Map<VectorXR> z12(int elem) {
        return Eigen::Map<VectorXR>(d_z12 + elem * (Quadrature::N_SHAPE_3243 / 2), Quadrature::N_SHAPE_3243);
    }

    __device__ Eigen::Map<VectorXR> const z12(int elem) const {
        return Eigen::Map<VectorXR>(d_z12 + elem * (Quadrature::N_SHAPE_3243 / 2), Quadrature::N_SHAPE_3243);
    }

    __device__ Eigen::Map<MatrixXR> F(int elem_idx, int qp_idx) {
        return Eigen::Map<MatrixXR>(d_F + (elem_idx * Quadrature::N_TOTAL_QP_3_2_2 + qp_idx) * 9, 3, 3);
    }
    ...
    __device__ Eigen::Map<MatrixXR> P(int elem_idx, int qp_idx) {
        return Eigen::Map<MatrixXR>(d_P + (elem_idx * Quadrature::N_TOTAL_QP_3_2_2 + qp_idx) * 9, 3, 3);
    }

    // Time-derivative of deformation gradient (viscous computation)
    __device__ Eigen::Map<MatrixXR> Fdot(int elem_idx, int qp_idx) {
        return Eigen::Map<MatrixXR>(d_Fdot + (elem_idx * Quadrature::N_TOTAL_QP_3_2_2 + qp_idx) * 9, 3, 3);
    }
    ...
    __device__ Eigen::Map<MatrixXR> P_vis(int elem_idx, int qp_idx) {
        return Eigen::Map<MatrixXR>(d_P_vis + (elem_idx * Quadrature::N_TOTAL_QP_3_2_2 + qp_idx) * 9, 3, 3);
    }
