...
GridMeshGenerator::GridMeshGenerator(Real X, Real Y, Real L, bool include_horizontal, bool include_vertical)
    ...
    if (std::abs(std::round(X_ / L_) * L_ - X_) > 1e-12 || std::abs(std::round(Y_ / L_) * L_ - Y_) > 1e-12) {
...
LinearConstraintBuilder::LinearConstraintBuilder(int n_dofs, const LinearConstraintCSR& initial)
    : n_dofs_(n_dofs),
    ...
    if (static_cast<int>(offsets_.size()) != initial.NumRows() + 1) {
        throw std::invalid_argument("LinearConstraintBuilder: initial offsets size mismatch");
    }
...
int LinearConstraintBuilder::AddRow(const std::vector<std::pair<int, Real>>& entries, Real rhs) {
    ...
    for (const auto& [col, val] : entries) {
        if (col < 0 || col >= n_dofs_) {
            throw std::out_of_range("LinearConstraintBuilder::AddRow: col out of range");
        }
        if (val == 0.0)
            continue;
...
void AppendANCF3243VectorEqualityConstraint(LinearConstraintBuilder& builder, int node_a, int node_b, int coef_slot) {
    if (coef_slot < 0 || coef_slot > 3) {
        throw std::out_of_range("AppendANCF3243VectorEqualityConstraint: coef_slot out of range");
    }
    for (int c = 0; c < 3; ++c) {
...
void AppendANCF3443VectorEqualityConstraint(LinearConstraintBuilder& builder, int node_a, int node_b, int coef_slot) {
    if (coef_slot < 0 || coef_slot > 3) {
        throw std::out_of_range("AppendANCF3443VectorEqualityConstraint: coef_slot out of range");
    }
    for (int c = 0; c < 3; ++c) {
...
void TrimInPlace(std::string& s) {
    ...
    s.erase(s.begin(), std::find_if(s.begin(), s.end(), [&](char ch) { return !is_space(ch); }));
    s.erase(std::find_if(s.rbegin(), s.rend(), [&](char ch) { return !is_space(ch); }).base(), s.end());
}
...
bool ReadANCF3243MeshFromFile(const std::string& path, ANCF3243Mesh& out, std::string* error) {
    out = ANCF3243Mesh();
    ...
    if (ReadNextRecord(file, line)) {
        ...
        if (!t.empty() && t[0] == "grid") {
            ...
            if (t.size() != 11 || t[1] != "nx" || t[3] != "ny" || t[5] != "L" || t[7] != "origin") {
    ...
    {
        const auto t = Tokenize(line);
        if (t.size() != 2 || t[0] != "nodes" || !ParseIntStrict(t[1], n_nodes) || n_nodes <= 0) {
    ...
    for (int i = 0; i < n_nodes; ++i) {
        ...
        const auto t = Tokenize(line);
        if (t.size() != 14) {
            SetError(error, "ReadANCF3243MeshFromFile: invalid node line (expected 14 tokens)");
            return false;
        }
        int node_id = -1;
    ...
    {
        const auto t = Tokenize(line);
        if (t.size() != 2 || t[0] != "elements" || !ParseIntStrict(t[1], n_elements) || n_elements <= 0) {
    ...
    for (int i = 0; i < n_elements; ++i) {
        ...
        const auto t = Tokenize(line);
        if (t.size() != 4) {
            SetError(error, "ReadANCF3243MeshFromFile: invalid element line (expected 4 tokens)");
            return false;
        }
        int elem_id = -1;
        int n0 = -1, n1 = -1;
        if (!ParseIntStrict(t[0], elem_id) || elem_id < 0 || elem_id >= n_elements) {
        ...
        if (!ParseIntStrict(t[2], n0) || !ParseIntStrict(t[3], n1) || n0 < 0 || n1 < 0 || n0 >= n_nodes ||
            n1 >= n_nodes) {
    ...
    {
        const auto t = Tokenize(line);
        if (t.size() != 2 || t[0] != "constraints" || !ParseIntStrict(t[1], n_constraints_header) ||
    ...
    for (int i = 0; i < n_constraints_header; ++i) {
        if (!ReadNextRecord(file, line)) {
            SetError(error, "ReadANCF3243MeshFromFile: unexpected EOF in constraints");
            return false;
        }
        const auto t = Tokenize(line);
        if (t.empty()) {
        ...
        if (t[0] == "pinned") {
            if (t.size() != 3) {
                SetError(error, "ReadANCF3243MeshFromFile: pinned expects 'pinned a b'");
                return false;
            }
            int a = -1, b = -1;
            if (!ParseIntStrict(t[1], a) || !ParseIntStrict(t[2], b) || a < 0 || b < 0 || a >= n_nodes ||
                b >= n_nodes) {
                SetError(error, "ReadANCF3243MeshFromFile: pinned node id out of range");
                return false;
            }
            AppendANCF3243VectorEqualityConstraint(builder, a, b, /*coef_slot=*/0);
        } else if (t[0] == "welded") {
            if (t.size() != 12) {
                SetError(error, "ReadANCF3243MeshFromFile: welded expects 'welded a b q00..q22'");
                return false;
            }
            int a = -1, b = -1;
            if (!ParseIntStrict(t[1], a) || !ParseIntStrict(t[2], b) || a < 0 || b < 0 || a >= n_nodes ||
            ...
            SetError(error, "ReadANCF3243MeshFromFile: unknown constraint type '" + t[0] + "'");
            return false;
        }
    }
...
bool ReadANCF3443MeshFromFile(const std::string& path, ANCF3443Mesh& out, std::string* error) {
    out = ANCF3443Mesh();
    ...
    for (int i = 0; i < n_nodes; ++i) {
        ...
        const auto t = Tokenize(line);
        if (t.size() != 14) {
            SetError(error, "ReadANCF3443MeshFromFile: invalid node line (expected 14 tokens)");
            return false;
        }
        int node_id = -1;
        if (!ParseIntStrict(t[0], node_id) || node_id < 0 || node_id >= n_nodes) {
    ...
    for (int i = 0; i < n_elements; ++i) {
        ...
        const auto t = Tokenize(line);
        if (t.size() != 9) {
            SetError(error, "ReadANCF3443MeshFromFile: invalid element line (expected 9 tokens)");
            return false;
        }
        int elem_id = -1;
        if (!ParseIntStrict(t[0], elem_id) || elem_id < 0 || elem_id >= n_elements) {
        ...
        if (!ParseIntStrict(t[5], n0) || !ParseIntStrict(t[6], n1) || !ParseIntStrict(t[7], n2) ||
            !ParseIntStrict(t[8], n3) || n0 < 0 || n1 < 0 || n2 < 0 || n3 < 0 || n0 >= n_nodes || n1 >= n_nodes ||
    ...
    for (int i = 0; i < n_constraints_header; ++i) {
        if (!ReadNextRecord(file, line)) {
            SetError(error, "ReadANCF3443MeshFromFile: unexpected EOF in constraints");
            return false;
        }
        const auto t = Tokenize(line);
        if (t.empty()) {
        ...
        if (t[0] == "pinned") {
            if (t.size() != 3) {
                SetError(error, "ReadANCF3443MeshFromFile: pinned expects 'pinned a b'");
                return false;
            }
            int a = -1, b = -1;
            if (!ParseIntStrict(t[1], a) || !ParseIntStrict(t[2], b) || a < 0 || b < 0 || a >= n_nodes ||
                b >= n_nodes) {
                SetError(error, "ReadANCF3443MeshFromFile: pinned node id out of range");
                return false;
            }
            AppendANCF3443VectorEqualityConstraint(builder, a, b, /*coef_slot=*/0);
        } else if (t[0] == "welded") {
            if (t.size() != 12) {
                SetError(error, "ReadANCF3443MeshFromFile: welded expects 'welded a b q00..q22'");
                return false;
            }
            int a = -1, b = -1;
            if (!ParseIntStrict(t[1], a) || !ParseIntStrict(t[2], b) || a < 0 || b < 0 || a >= n_nodes ||
            ...
            SetError(error, "ReadANCF3443MeshFromFile: unknown constraint type '" + t[0] + "'");
            return false;
        }
    }
